...
describe("Blacklist Tests:", function () {
  ...
  it("Blacklisted Avatar check", function () {
    assert.equal(
      BLUtil.isBlacklistedAvatar(BLUtil.blacklistedavatars[1]),
      true
    );
    assert.equal(
      BLUtil.isBlacklistedAvatar("42693a500e622d60fae045cda2e5261f"),
      false
    );
    assert.equal(
      BLUtil.isBlacklistedAvatar("ec8768f48105c1219db7f05cfd7d6b84"),
      false
    );
  });
  ...
  it("Brand New Acc detection", function () {
    assert.equal(BLUtil.checkIfUserIsNew(Date.now() - 1.44e7, null), true);
    assert.equal(BLUtil.checkIfUserIsNew(Date.now() - 1.08e7, null), true);
    assert.equal(BLUtil.checkIfUserIsNew(Date.now() - 2.592e7, null), false);
    assert.equal(BLUtil.checkIfUserIsNew(Date.now() - 3.456e8, null), false);
  });
});
