...
function isFakeBitmex(username){
    return (username.includes("BitMex") || username.includes("Bitmex")) && !username.includes("Trader") 
}
...
function CheckBLBotImper(user,isBot = false,fCheck = true){
    var toreturnbool =  user != undefined  && !isBot 
...
function isLibraSpam(msg,fCheck = true){
    var Returnval = containsStringInarray(BLConsts.LibraDetects,msg.toLowerCase())
    if(!(Returnval) && fCheck){
...
function CheckBLMatch(username,useravatar=null ,isBot=false,userid=0){
 ...
 function CheckBLMatchMember(member){
    ...
    return isBlacklistedAvatar(useravatar) || hasBlacklistedUsername(username,isBot) || CheckBLBotImper(username,isBot) || checkIfIDIsBlacklised(userid) || checkIfUserIsNew(member.user.createdTimestamp,useravatar);
}
module.exports = {
    isBlacklistedAvatar,
    isFakeBitmex,
